...
import { NTableController } from "./NTableController.js";
...
export class NTableView extends ProtoView
{
    ...
    resolve(type, scope = 'NTableCells')
    {
    ...
    header()
    {
        ...
        const items = Arr.each(this.scope.childs, (column, index) => {
    ...
    context()
    {
        ...
        groupProps['onUpdate:modelValue'] = (value) => {
            scope.update('visible', value);
        };

        const values = Arr.each(scope.childs, (column) => {
    ...
    column(column, index)
    {
        ...
        props['onUpdate:width'] = (width) => {
            data.looseWidth[column.uid] = width;
        };

        props['onUpdate:modelValue'] = (width) => {
    ...
    popover(column)
    {
        ...
        applyProps.onClick = () => {
            scope.applyColumnFilter(column);
        };
    ...
    body()
    {
        ...
        return this.comp('n-draglist', props, (node) => {
    ...
    rows(node)
    {
        ...
        const columns = Arr.filter(scope.childs, (column) => {
            return Arr.has(data.visible, column.uid);
        });

        return Arr.each(columns, (column, index) => {
            return this.cell(node, column, index);
        });
    }

    cell(node, column, index)
    {
        const [{ scope }, { data }] = [
            this.scope.unpack(), column.unpack()
        ];
        ...
        if ( !this.buffer[column.uid] ) {
            this.buffer[column.uid] = cell;
        }
