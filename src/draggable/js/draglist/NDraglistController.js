...
import { Arr, Dom, Hash, Mix, Num, Obj, Run } from "@kizmann/pico-js";
import { ProtoController, Pointer } from "../../../root/index.js";
import { NDraglistProps } from "./NDraglist.js";
import { NDraglistView } from "./NDraglistView.jsx";
import { NDraglistData } from "./NDraglistData.js";
...
export class NDraglistController extends ProtoController
{
    ...
    setup()
    {
        ...
        this
            ...
            .linkProp('rootSkip')
            .linkProp('safezone')
            .linkProp('transformDrop')
            .linkProp('insertNode')
            .linkProp('removeNode')
            .linkProp('allowSelect')
            .linkProp('allowDrop')
            .linkProp('allowDrag')
            .linkProp('scrollPortal')
            .linkProp('renderHandle')
            .linkProp('renderExpand')
            .linkProp('renderSelect');
    ...
    onMounted()
    {
        ...
        config.dragmove = (...args) => {
            return this.onDragmove(...args);
        }

        config.dragdrop = (...args) => {
    ...
    buildVisibles()
    {
        ...
        visibles = Arr.filter(visibles, (node) => {
    ...
    buildRelation()
    {
        let relation = Arr.find(this.data.virtuals, (val) => {
    ...
    buildVirtuals(items = null, ...args)
    {
        if ( items == null ) {
            items = Arr.clone(this.data.items);
        }

        items = Arr.reduce(items, (merge, item, index) => {
    ...
    extractValues(merge, item, index, depth = 0, path = ['items'], prev = [])
    {
        const { data } = this.unpack();
    ...
    onDragmove(e, result, config, els)
    {
        const { data } = this.unpack();
    ...
    unlinkNodes(clone, result, config)
    {
    ...
    removeNodes(clone, result, config)
    {
        if ( !this.data.removeNode ) {
            return clone;
        }
        ...
        const fn = (value, prop = 'items') => {
            return Arr.filter(value?.[prop]);
        };

        clone.items = Arr.recursive(fn(clone), childProp, (node) => {
    ...
    appendNodes(clone, result, config)
    {
    ...
    insideNodes(clone, result, config)
    {
        const { data } = this.unpack();
    ...
    beforeNodes(clone, result, config)
    {
        const { data } = this.unpack();
    ...
    afterNodes(clone, result, config)
    {
        const { data } = this.unpack();
    ...
    nodeDragstart(e, item)
    {
        ...
        const virtuals = Arr.each(selected, (uid) => {
            return Arr.find(data.virtuals, { uid });
        });

        const items = Arr.each(virtuals, (value) => {
    ...
    nodeDragmove(e, result, config, els)
    {
        const { data } = this.unpack();
        ...
        let rainbow = Arr.each(config.items ?? [], (el) => {
    ...
    nodeAllowDrop(target, result, config)
    {
        ...
        let rainbow = Arr.each(config.items, (node) => {
